...
const OtpVerificationForm = () => {
  ...
  const verifyOtp = async (email: string, otp: string) => {
    await axios
      .get("/api/forgot-password", { params: { email, otp } })
      .then((res) => res.data)
      .then((data) => {
        sessionStorage.setItem("for-pass", JSON.stringify(data?.data));
        setIsTimerRunning(false);
        toast("OTP Verified! redirecting...", {
          type: "success",
          delay: 2000,
    ...
    <div className="w-full flex flex-col justify-center items-center gap-y-16">
      ...
      <div className=" lg:w-6/12 md:7/12 w-8/12 shadow-3xl rounded-xl">
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="p-12 md:p-24">
          ...
          >
            {otpSent ? "Verify OTP" : isResending ? "Resend OTP" : "Send OTP"}
          </button>
        </form>
      </div>
    </div>
